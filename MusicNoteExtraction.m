%extract notes from audio file
%input file: FurElise_Slow.mp3

%% set up song
clear; clc; clf; close all

mute = false; % if false then audio will be played throughout program
[song,Fs] = audioread('FurElise_Slow.mp3');
Fs = Fs*4;   % speed up song
figure, plot(song(:,1)), title('Fur Elise, entire song')

%% set parameters (will be change based on song)
t1 = 2.9e6; t2 = 4.9e6;

% analyzing a window of the song
y = song(t1:t2);
[~,n] = size(y);
t = linspace(t1,t2,n);
...
clc
m = 20;
Fsm = round(Fs/m); % downsampled frequecy 
p = floor(n/m);    % downsampled size
y_avg = zeros(1,p);
for i = 1:p
